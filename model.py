...
class Eapp(nn.Module):
    def __init__(self):
        ...
        self.fc = torch.nn.Linear(512, COMPRESS_DIM)
    ...
    def forward(self, x):
        ...
        logging.debug(f"After hourglass: {feature_map.shape}")  # [1, 512, 64, 64]
        
        vs = self.conv1x1(feature_map)
        logging.debug(f"After conv1x1: {vs.shape}")  # [1, 96, 64, 64]
        
        vs = vs.view(vs.size(0), 96, 16, *vs.shape[2:])
        logging.debug(f"reshape to C96 x D16: {vs.shape}")  # [1, 96, 16, 64, 64]
        
        # Apply 3D ResBlock layers
        ...
        logging.debug(f"After 3D ResBlocks: {vs.shape}")  # [1, 96, 16, 64, 64]

        # Second part
        es = F.adaptive_avg_pool2d(feature_map, (1, 1)).view(x.size(0), -1)
        logging.debug(f"Adaptive avg pool: {es.shape}")  # [1, 512]
        
        es = self.fc(es)
        logging.debug(f"Final es shape: {es.shape}")  # [1, COMPRESS_DIM]
