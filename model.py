...
UPDATE
According to the paper, the input to the G3D network (vc) has a shape of (batch_size, 96, 16, 64, 64), where:

batch_size is the number of samples in the batch
96 is the number of channels in the volumetric tensor
16 is the depth dimension of the volumetric tensor
64 x 64 is the spatial resolution of the volumetric tensor

The output of the G3D network (vc2d) has the same shape as the input (batch_size, 96, 16, 64, 64). This output volume is then warped using the warping field wc2d generated by the warping generator Wc2d to impose the driving motion.
...
class G3d(nn.Module):
    def __init__(self, in_channels):
        ...
        self.gn_in = nn.GroupNorm(num_groups=32, num_channels=64)
        
        self.res_block1 = ResBlock3D(64, 128)
        self.res_block2 = ResBlock3D(128, 256)
        self.res_block3 = ResBlock3D(256, 512)
        
        self.res_block4 = ResBlock3D(512, 256)
        self.res_block5 = ResBlock3D(256, 128)
        self.res_block6 = ResBlock3D(128, 64)
        
        self.conv_out = nn.Conv3d(64, 96, kernel_size=3, padding=1)
        
        self.shortcut1 = nn.Conv3d(64, 256, kernel_size=1)
        self.shortcut2 = nn.Conv3d(128, 128, kernel_size=1)
        #self.proj_x5 = nn.Conv3d(128, 512, kernel_size=1)
        
    def forward(self, x):
        x_in = F.relu(self.gn_in(self.conv_in(x)))
        
        x1 = self.res_block1(x_in)
        ...
        shortcut1 = self.shortcut1(x_in)
        shortcut2 = self.shortcut2(x1)
        x4 += shortcut1
        x5 += shortcut2
        
        x_out = self.conv_out(x6)
        return x_out
...
class WarpGeneratorS2C(nn.Module):
    def __init__(self, num_channels):
        ...
        # Adaptive parameters are generated by multiplying these sums with learned matrices.
        self.adaptive_matrix_gamma = nn.Parameter(torch.randn(2048, 4)).to(device)
        self.adaptive_matrix_beta = nn.Parameter(torch.randn(2048, 4)).to(device)
...
class Gbase(nn.Module):
    def __init__(self):
        ...
        self.G3d = G3d(in_channels=96).to(device)
